...
extern "C" fn on_message_trampoline(msg: *const c_char, raw: *mut c_void) -> bool {
    ...
    if let Some(w) = bb.0.upgrade() {
        rc.borrow_mut()(w, &unsafe { shot_str(msg).unwrap() });
    }
...
impl Webview {
    ...
    pub fn page_title(&self) -> String { unsafe { take_str(saucer_webview_page_title(self.as_ptr())).unwrap() } }

    /// Checks whether DevTools is opened.
    pub fn dev_tools(&self) -> bool { unsafe { saucer_webview_dev_tools(self.as_ptr()) } }

    /// Gets the URL of the current page.
    pub fn url(&self) -> String { unsafe { take_str(saucer_webview_url(self.as_ptr())).unwrap() } }
    ...
    pub fn title(&self) -> String { unsafe { take_str(saucer_window_title(self.as_ptr())).unwrap() } }
